...
//if (userid == '586b5bbe935a6d19040c5447' | userid == '5878b000d1f7c0220c1d2903') {
  if (req.user.permission == 'superadmin' | req.user._id == dataset.userID) { 
    console.log('CREATE SCADA : Superadmin',dataset)
    if (dataset.elementID !='') {
    ...
    var create = new scadaModel(dataset);  
...
if (err) {
  ...
  res.send({redirect: '/admin'});
        ...
        console.log('CREATE SCADA : new items are here' , dataset.entry)
        var create = new scadaModel(dataset);  
        create.save(function (err, doc) {  
          if (err) {
            console.log('CREATE SCADA : Error flag',err)
            res.send(err);
          } else{
            console.log('CREATE SCADA : step 6 - Save point for the dataset doc:',doc)
            res.send({redirect: '/admin'});
          }
        });
      }
